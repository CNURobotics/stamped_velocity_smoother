 ...
 *
 * License: BSD
 *   https://raw.github.com/CNURobotics/stamped_velocity_smoother/LICENSE
 **/
/*****************************************************************************
 ** Includes
 *****************************************************************************/
...
#include <stamped_velocity_smoother/paramsConfig.h>

#include <boost/thread.hpp>

#include "stamped_velocity_smoother/velocity_smoother_nodelet.hpp"
...
#define IS_ZERO_VEOCITY(a)   ((a.twist.linear.x == 0.0) && (a.twist.angular.z == 0.0))

/*****************************************************************************
** Namespaces
*****************************************************************************/

namespace stamped_velocity_smoother {
...
void VelocitySmoother::reconfigCB(stamped_velocity_smoother::paramsConfig &config, uint32_t level)
{
...
void VelocitySmoother::twistCB(const geometry_msgs::Twist::ConstPtr& msg)
{
    boost::unique_lock<boost::mutex> scoped_lock(mutex);
    last_target_vel_msg.header.stamp = ros::Time::now();
    velocityTarget(*msg);
}
void VelocitySmoother::twistStampedCB(const geometry_msgs::TwistStamped::ConstPtr& msg)
{
    boost::unique_lock<boost::mutex> scoped_lock(mutex);
    last_target_vel_msg.header.stamp = msg->header.stamp;
    velocityTarget(msg->twist);
}

void VelocitySmoother::velocityTarget(const geometry_msgs::Twist& msg)
{
...
void VelocitySmoother::odometryCB(const nav_msgs::Odometry::ConstPtr& msg)
{
  if (robot_feedback == ODOMETRY)
    current_vel.twist = msg->twist.twist;

  // ignore otherwise
}

void VelocitySmoother::robotVelCB(const geometry_msgs::TwistStamped::ConstPtr& msg)
{
...
void VelocitySmoother::spin()
{
  ...
  ROS_WARN("Starting spin loop for velocity smoother ...");
  while (! shutdown_req && ros::ok())
  {
    {   // update the data from ROS thread
        boost::unique_lock<boost::mutex> scoped_lock(mutex);
        target_vel = last_target_vel_msg;
    }
        ...
        ((ros::Time::now() - last_cb_time).toSec() > std::min(3.0*cb_avg_time, 0.5)))
    {
      ...
      if (IS_ZERO_VEOCITY(target_vel) == false)
      {
        ROS_WARN_STREAM("Velocity Smoother : input got inactive leaving us a non-zero target velocity ("
              << target_vel.twist.linear.x << ", " << target_vel.twist.angular.z << "), zeroing...[" << name << "]");
        target_vel.header.stamp = ros::Time::now();
        target_vel.twist = ZERO_VEL_COMMAND;
      }
    }

    if ((robot_feedback != NONE) && (input_active == true) && (cb_avg_time > 0.0) &&
        (((ros::Time::now() - last_cb_time).toSec() > 5.0*cb_avg_time)     || // 5 missing msgs
          (std::abs(current_vel.twist.linear.x  - last_cmd_vel.twist.linear.x)  > 0.2) ||
          (std::abs(current_vel.twist.angular.z - last_cmd_vel.twist.angular.z) > 2.0)))
    {
      ...
      if ( !quiet ) {
        ...
        ROS_WARN_STREAM("Velocity Smoother : using robot velocity feedback " <<
                        ...
                        current_vel.twist.linear.x  - last_cmd_vel.twist.linear.x << ", " <<
                        current_vel.twist.angular.z - last_cmd_vel.twist.angular.z << ", [" << name << "]"
                        );
      }
      last_cmd_vel.twist = current_vel.twist;
      last_cmd_vel.header= current_vel.header;
    }

    geometry_msgs::TwistStampedPtr cmd_vel;

    if ((target_vel.twist.linear.x  != last_cmd_vel.twist.linear.x) ||
        (target_vel.twist.angular.z != last_cmd_vel.twist.angular.z))
    {
      // Try to reach target velocity ensuring that we don't exceed the acceleration limits
      cmd_vel.reset(new geometry_msgs::TwistStamped());
      cmd_vel->header = target_vel.header;
      cmd_vel->twist  = target_vel.twist;

      double v_inc, w_inc, max_v_inc, max_w_inc;

      v_inc = target_vel.twist.linear.x - last_cmd_vel.twist.linear.x;
      if ((robot_feedback == ODOMETRY) && (current_vel.twist.linear.x*target_vel.twist.linear.x < 0.0))
      {
      ...
      {
        max_v_inc = ((v_inc*target_vel.twist.linear.x > 0.0)?accel_lim_v:decel_lim_v)*period;
      }

      w_inc = target_vel.twist.angular.z - last_cmd_vel.twist.angular.z;
      if ((robot_feedback == ODOMETRY) && (current_vel.twist.angular.z*target_vel.twist.angular.z < 0.0))
      {
        // countermarch (on robots with significant inertia; requires odometry feedback to be detected)
      ...
      {
        max_w_inc = ((w_inc*target_vel.twist.angular.z > 0.0)?accel_lim_w:decel_lim_w)*period;
      }
      ...
      if (std::abs(v_inc) > max_v_inc)
      {
       ...
       cmd_vel->twist.linear.x  = last_cmd_vel.twist.linear.x  + sign(v_inc)*max_v_inc;
      }

      if (std::abs(w_inc) > max_w_inc)
      {
        // we must limit angular velocity
       cmd_vel->twist.angular.z = last_cmd_vel.twist.angular.z + sign(w_inc)*max_w_inc;
      }
    ...
    else if (input_active == true)
    {
      ...
      cmd_vel.reset(new geometry_msgs::TwistStamped());
      cmd_vel->twist = last_cmd_vel.twist;
      cmd_vel->header.stamp = ros::Time::now();
      smooth_vel_pub.publish(cmd_vel);
...
{
  // Dynamic Reconfigure
  ROS_WARN("Initializing the VelocitySmoother class ");
  dynamic_reconfigure_callback = boost::bind(&VelocitySmoother::reconfigCB, this, _1, _2);

  dynamic_reconfigure_server = new dynamic_reconfigure::Server<stamped_velocity_smoother::paramsConfig>(nh);
  ...
  raw_in_vel_sub  = nh.subscribe("raw_cmd_vel",   1, &VelocitySmoother::twistCB, this);
  smooth_vel_pub  = nh.advertise <geometry_msgs::TwistStamped> ("smooth_cmd_vel", 1);
  ROS_WARN(" initialized velocity smoother");
  return true;
}
...
public:
  ...
  virtual void onInit()
  {
    ROS_WARN(" VelocitySmootherNodelet initialization");
    ...
    NODELET_WARN_STREAM("Velocity Smoother : initialising nodelet...[" << name << "]");
    vel_smoother_.reset(new VelocitySmoother(name));
    if (vel_smoother_->init(ph))
    {
      NODELET_WARN_STREAM("Velocity Smoother : nodelet initialised [" << name << "]");
      worker_thread_ = boost::thread(boost::bind(&VelocitySmoother::spin, vel_smoother_));
    }
    else
    {
...
PLUGINLIB_EXPORT_CLASS(stamped_velocity_smoother::VelocitySmootherNodelet, nodelet::Nodelet);
